...
<% 
	...
	ResultSet rs=stmt.executeQuery("select state from contract where id_reservation='"+rid+"'");
%>
<script type="text/javascript">
       function check(){
            if(document.myform.tpersoncard.value==""){
              window.alert("身份证号不能为空！");
              return ;
            }
            else if(document.myform.tphone.value=="")
            {
                window.alert("电话不能为空！");
                return ;
            }
...
<script type="text/javascript">
	function mm()
	{
		response.setHeader("refresh","0;URL=tenant.jsp");
	}
</script>
<!-- <script type="text/javascript">
	function dis()
	{//js表单验证方法
		alter("您已提交合同，请等待出租者签署");
	}
</script>  -->
	<h1>合同</h1>
	<hr>
	<p>
		<% if(rs.next())
		{
			if(rs.getInt(1)!=1&&rs.getInt(1)!=3)	
			{
				out.print("出租者还未签合同");
			}
			else if(rs.getInt(1)==1&&rs.getInt(1)!=3)
			{
				out.print("出租者已签合同");
			}
		}
		...
		<form id="myform" name="myform" action="handlecontract.jsp" method="post" >
			<table>
			 	<tr>
			 	<td>  承租方(简称乙方) 身份证号：  </td>
				<td><input type="text" name="tpersoncard" id="tpersoncardid" > </td>				
				</tr>
				<tr>
			 	<td>电话：  </td>
				<td><input type="text" name="tphone" id="tphoneid" > </td>				
				</tr>
				<tr>  <td colspan="2">出租方(简称甲方) 身份证号： </td> </tr>
				<tr>  <td colspan="2">电话： </td> </tr>
				...
				<tr>
					...
					<td><input type="button" onclick="window.location.href='tenant.jsp'" value="返回">	</td>
				</tr>
			</table>
		</form>
